...
$('.size-button').on('click', function() {
    ...
    $.ajax({
            ...
            success: function(response) {
                $('#price-heading').text('$' + response.price);
...
$(document).on("click", ".confirmDelete", function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          window.location.href=base_url+'/admin/'+module+'/delete/'+module_id;
...
$('#reserve_btn').click(function () {
    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            if (resp.message == 'reserved') {
                Swal.fire({
                    ...
                    html: "You have reserved the jersey number <span style='color:#3085d6'>" + resp.jersey_number + "</span>  for <span style='color:#3085d6'>30 days </span>",
                    icon: 'success',
                    // showCancelButton: true,
            ...
            if (resp.message == 'out_of_stock') {
                Swal.fire({
                    title: "Out of stock",
                    html: "The jersey number <span style='color:#f27474'> " + resp.jersey_number + " </span> is out of stock",
                    icon: 'error',
                });
            ...
            if (resp.message == 'already_reserved') {
                Swal.fire({
                    ...
                    html: "The jersey number <span style='color:#f27474'> " + resp.jersey_number + " </span> is already reserved. Try another group",
                    icon: 'error',
                });
            ...
            if (resp.message == 'out_of_stock_for_10_to_13') {
                Swal.fire({
                    title: "Not Available",
                    html: "The jersey number <span style='color:#f27474'> " + resp.jersey_number + " </span> is not available",
                    icon: 'error',
                });
        ...
        error: function (request, status, error) {
            ...
            $('[name="name"]').next('span').html(request.responseJSON.errors.name);
            // $('[name="jersey_number"]').next('span').html(request.responseJSON.errors.jersey_number);
            $('#jersey_number_error_msg').html(request.responseJSON.errors.jersey_number);
            $('[name="age_group"]').next('span').html(request.responseJSON.errors.age_group);
            $('input[type=radio][name=size]').next('span').html(request.responseJSON.errors.size);
